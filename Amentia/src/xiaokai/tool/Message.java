...
public class Message {
	...
	private String[] Key = { "{n}", "{RandColor}", "{ServerName}", "{PluginName}", "{Time}", "{Date}" };
	private String[] Data = {};
	private Config Message;

	public Message(YouAreFool mis) {
		this.mis = mis;
		Message = new Config(new File(mis.getDataFolder(), YouAreFool.MsgName), 2);
	}

	private void load() {
		Data = new String[] { "\n", Tool.getRandColor(), mis.getServer().getMotd(), mis.getName(), Tool.getTime(),
				Tool.getDate() };
	}
